...
;;; ;------------------------------------------------------
;;; ;------------------------------------------------------
;;; ; readFile: This function reads the contents
;;; ;          of a file into a buffer.  The address of the
;;; ;          string containing the filename is passed in
;;; ;          eax.  The function returns the address of the
;;; ;          buffer containing the bytes found in the file,
;;; ;          in ecx.  It also returns the number of bytes
;;; ;          read in edx.
;;; ;------------------------------------------------------
;;; ;------------------------------------------------------
	section		.data
handle		dd	0
fileName	dd	0	
errorMsgOpen	db	"Could not open file.", 10
errorMsgOpenL	equ	$-errorMsgOpen
noBytes3        dd	0
bufferSave	dd	0
noCharsSave	dd	0
...
_readFile:	mov	dword[fileName], eax ; save eax
		mov	dword[bufferSave], ebx ; save ebx
		mov	dword[noCharsSave], ecx ; save ecx
		...
		;; test to see if successful operation
	        test    eax,eax
	        jns     readFileContents
	        mov	ecx, errorMsgOpen
		mov	edx, errorMsgOpenL
		call	_printString
...
readFileContents:
		...
		;; read bytes from file into buffer
        	mov     eax, SYS_READ
	        mov     ebx, dword[handle]	; file descriptor in bx
	        mov     ecx, dword[bufferSave]	; buffer address
	        mov     edx, 1000		; max number of bytes to read
	        int     0x80
		mov	ebx, dword[noCharsSave]
	        mov     [ebx], eax		; save number bytes read
...
closeFile:	;; close the file
		...
		;; returned all the registers to their original values
		popad

		;; return buffer3 in ecx, and noBytes3 in edx
		ret
...
;;; ;------------------------------------------------------
;;; ;------------------------------------------------------
;;; ; _getString: returns a string typed at the keyboard
;;; ;             by the user.  The user must type ^D or
;;; ;		  or RETURN to stop entering the string.
;;; ; The address of the string is returned in ecx
;;; ; The number of chars read is returned in edx
;;; ;------------------------------------------------------
;;; ;------------------------------------------------------
_getString:
		section	.bss
...
.loop1:
		...
		jmp     .loop1
.done:
		mov	dword[noChars], edi
		popad
;;; return string in ecx (address) and edx (num chars)
		mov	ecx, buffer2
